...
export default function page() {
  ...
  return (
    <>
      ...
      <div className="common-padding pt-0! relative z-2 -mt-50 md:-mt-75">
        <Container>
          <motion.div
            ...
            <div className="p-5 md:p-8 lg:p-11 border border-solid border-white/12 bg-[linear-gradient(180deg,rgba(255,255,255,0.04)_0%,rgba(255,255,255,0.00)_100%)] xl:max-w-160 w-full rounded-2xl md:rounded-3xl lg:rounded-4xl">
              <div className="mb-6 md:mb-10 lg:mb-12">
                <Comment />
                <h4 className='text-2xl leading-none mt-4'>Talk to sales</h4>
                <p className='text-base mt-3'>Complete the thought build better insights.</p>
              </div>
              <div className="flex flex-col gap-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Input type="text" label="Name" placeholder="Jenny Wilson" />
                  <Input type="email" label="Email" placeholder="example@gmail.com" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Input type="text" label="Company" placeholder="Louis Vuitton" />
                  <Input type="text" label="Country" placeholder="USA" />
                </div>
                <Input type="textarea" label="How can we help?" placeholder="Tell us about your enterprises needs" />
                <button className='btn w-full md:w-max md:min-w-65'><span>Get In Touch</span></button>
              </div>
            </div>
            <div className="flex flex-wrap md:flex-nowrap flex-row xl:flex-col gap-4 w-full xl:max-w-93">
              {items.map((item, index) => (
                <div className="w-full flex grow flex-col items-center justify-center gap-4 py-8 lg:py-10 md:h-full text-center border border-solid border-white/12 bg-[linear-gradient(180deg,rgba(255,255,255,0.04)_0%,rgba(255,255,255,0.00)_100%)] rounded-2xl md:rounded-3xl lg:rounded-4xl" key={index}>
                  <div className="flex items-center justify-center">{item.icon}</div>
                  <h4 className='text-2xl'>{item.title}</h4>
                  <p className='text-base text-[#B4B4B4]'>{item.des}</p>
                </div>
              ))}
            </div>
          </motion.div>
